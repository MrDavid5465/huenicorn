...
namespace Huenicorn::Serialization
{
  ...
  {
    static void from_json(const Json& jsonUV, Imaging::UV& uv)
    {
      uv = Imaging::UV{
        jsonUV.at("x"),
        jsonUV.at("y")
    ...
    static void to_json(Json& jsonUv, const Imaging::UV& uv)
    {
      jsonUv = {
        {"x", uv.x},
        {"y", uv.y}
  ...
  {
    static void to_json(Json& jsonUvs, const Imaging::UVs& uvs)
    {
      jsonUvs = {
        {"uvA", uvs.min},
        {"uvB", uvs.max}
    ...
    static void from_json(const Json& jsonUVs, Imaging::UVs& uvs)
    {
